...
class GoogleGAL_Http_REST {
	 ...
	 * @throws GoogleCFA_Service_Exception on server side error (ie: not authenticated,
	 ...
	 *
	 * @static
	 * @throws GoogleCFA_Service_Exception
	...
	public static function decodeHttpResponse( $response, GoogleGAL_Client $client = null ) {
		...
		if ( ( intVal( $code ) ) >= 300 ) {
			...
			throw new GoogleCFA_Service_Exception( $err, $code, null, $errors );
		}
		...
		if ( $code != '204' ) {
			...
			if ( $decoded === null || $decoded === '' ) {
				...
				throw new GoogleCFA_Service_Exception( $error );
			}
